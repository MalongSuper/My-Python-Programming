...
def load_data():
    if os.path.exists('student_data.pkl'):  # Check if the file exists
        with open('student_data.pkl', 'rb') as file:  # Open the file
            try:
...
def save_to_excel():  # Function to save the data to an Excel file
    ...
    data.to_excel("student_management_system.xlsx", index=False)
